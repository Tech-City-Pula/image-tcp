...
export default function Home() {
    const [generatedImage, setGeneratedImage] = useState<imagesType[] | null>(
        null
    );
    ...
    const handleNumberOfImagesChange = (
    ...
    useEffect(() => {
        const promptTimer = setTimeout(
            () => promptError && setPromptError(false),
            3000
        );
    ...
    useEffect(() => {
        const numberTimer = setTimeout(
            () => numberOfImagesError && setNumberOfImagesError(false),
            3000
        );
        ...
        <>
            ...
            <main>
                <div className='flex flex-col w-full h-screen gap-5 justify-center items-center p-[20px]'>
                    <p className='uppercase text-xl font-thin font-mono'>
                        Image Generator
                    </p>
                    <UserInput
                        prompt={prompt}
                    ...
                    {promptError && (
                        <div className='text-red-400'>
                            Please describe the picture you want to generate.
                        </div>
                    )}
                    {numberOfImagesError && (
                        <p className='text-red-400'>
                            maximum amount of pictures generated can be 10
                        </p>
                    )}
                    <div className='flex flex-row gap-4'>
                        {buttons.map((button, index) => (
                            <button
                                key={index}
                                onClick={() => setSelectedSizeOfImage(button)}
                                className={`${
                                    selectedSizeOfImage === button
                                        ? 'bg-blue-200 scale-125'
                                        : ''
                                } text-black bg-blue-200 px-4 py-1 text-center rounded-full text-sm sizeButton`}
                            >
                                {button}
                            </button>
                        ))}
                    ...
                    <button
                        ...
                        className='generateButton rounded-full px-[25px] py-[5px] bg-blue-400 font-bold uppercase text-xl'
                    >
                        Generate!
                    </button>
                    {loading && (
